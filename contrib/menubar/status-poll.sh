#!/bin/sh
# Thin synctr status --json consumer. The macOS menu bar extra parses the
...
print(f"{rclone_s} profiles={len(profiles)} ok={ok} fail={fail} never={never} running={running}")
' <<EOF
$json
EOF
